...
package com.hurlant.crypto.tls {
	...
	public class TLSEngine extends EventDispatcher {
		...
		private var _gotCertRequest:Boolean = false;
		...
		private function parseHandshake(p:ByteArray):ByteArray {
			...
			switch (type) {
				...
				case HANDSHAKE_CERTIFICATE_REQUEST:
					...
					//actually, it is now --Nathan Fritz
					_gotCertRequest = true; // so that when we get the hello_done, we can send the certificate
					break;
				case HANDSHAKE_HELLO_DONE:
					if (!enforceClient()) break;
					if(_gotCertRequest) {
						sendClientCertificate(); // ejabberd wants this --Nathan Fritz
					}
					sendClientAck();
					break;
		...
		//added by Nathan Fritz --- send back to Hurlant?
		private function sendClientCertificate():void {
			var cert:ByteArray = _config.certificate;
			if (cert == null) cert = new ByteArray();
			var len:uint = cert.length;
			var len2:uint = len + 0; // this implies we only ever send 1 certificate. XXX okay for now.
			var rec:ByteArray = new ByteArray;
			rec.writeByte(len2>>16);
			rec.writeShort(len2&65535);
			rec.position = 0;
		...
		private function loadCertificates(certs:Array):void {
			//TODO: make various aspects of authenticity checks optional --Nathan Fritz
			//accepts any domain and self signed certs right now... bad!
			...
			if (firstCert.isSigned(_store, _config.CAStore)) {
				...
				if (_otherIdentity==null) {
					...
					if (firstCert.getCommonName()==_otherIdentity) {
						...
						//TODO Need to * match
						trace('Invalid cert name, ' + firstCert.getCommonName());
						_otherCertificate = firstCert;
					}
				}
			} else {
				...
				_otherCertificate = firstCert;
				trace("Warning: Cannot verify certificate.");
			}
		}
